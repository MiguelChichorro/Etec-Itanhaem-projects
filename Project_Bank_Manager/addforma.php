  ...
  <body>
    <div class="container-fluid">
      ...
      <div class="row" style="margin-left: 20px; margin-right: 20px;">
        <div class="col-sm-6">
          <form method="post">
            <h1>Adicione uma nova forma</h1>
            <hr>
            <div class="form-label-group">
              <label for="inputaddfor">Add</label>
              <input type="text" id="inputaddfor" class="form-control" name="for" required autofocus>
              <input class="btn btn-outline-success btn-lg" type="submit" value="Enviar"></input>
            </div>
           <?php
            if (isset($_POST['for'])) {
              $sql = "insert into tb_forma value(null,'". $_POST['for'] . "','" . $_SESSION['COD_USER']."')";
            }
        ...
        <div class="col-sm-6">
          <div id="table">
            <table class="table">
              ...
              <?php
              if (isset($_SESSION['COD_USER'])) {
                $query = "SELECT * FROM tb_forma as fo
                where
                fo.id_usuario = " . $_SESSION['COD_USER'] ." ORDER BY cd_forma DESC";
                if ($result = $mysqli->query($query)) {
                  while ($obj = $result->fetch_object()) {
                    echo "<tbody>
                    <tr>
                      <td>$obj->ds_forma</td>
                      <td><button id='delet'><a href=exclu.php?fo=$obj->cd_forma><img src='delet.png' width='20' height='20'></a></button></td>
                    </tr>";
                  }
                }
              else{
